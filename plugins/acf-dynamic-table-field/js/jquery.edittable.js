...
(function ($, window, i) {
    ...
    $.fn.editTable = function (options) {
        ...
        var s = $.extend({
                ...
                maxRows: 999,
                first_row: true,
                //Geet/Anagram adding sort on off option
                sortable: true,
                row_template: false,
        ...
        function buildRow(data, len) {
			...
			//Anagram/Geet - Added remove class to TD below. Also order TD
			var sorttd='';

			if (s.sortable ) {
            	sorttd = '<td class="order"></td>';
            	//$table.find('thead tr').prepend('<th></th>');
            	//$table.find('tbody tr').prepend('<td class="order"></td>');
            }
            return $('<tr/>', {
                html: sorttd+ rowcontent + '<td class="remove"><a class="addrow icon-button" href="#">+</a> <a class="delrow icon-button" href="#">-</a></td>'
            });
        ...
        function fillTableData(data) {
            ...
            //End Anagram/Geet - Add sort option
        ...
        function exportData() {
            ...
            $table.find('tbody tr').each(function () {
                ...
                row += 1;
                data[row] = [];
				//Edited by anagram/geet ignore two classes for columns (added ,.order)
                $(this).find('td:not(.remove,.order)').each(function (i, v) {
