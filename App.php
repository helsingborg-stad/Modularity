...
class App
{
    ...
    public function __construct()
    {
        ...
        add_filter('do_shortcode_tag', array($this, 'uniqifyCaptionIds'), 10, 3);
    }
    ...
    public function addAdminMenuPage()
    {
		...
		public function uniqifyCaptionIds($output, $tag, $attr) {
      if ($tag == "caption") {
        $divId = $attr['id'];
        $idNumber = \explode('_', $divId)[1];
        $captionId = "caption-attachment-$idNumber";
        $uniqueId = rand(1000, 9999);

        $output = str_replace("id=\"$divId\"", "id=\"$divId-{$uniqueId}\"", $output);
        $output = str_replace("id=\"$captionId\"", "id=\"$captionId-{$uniqueId}\"", $output);
        $output = str_replace("aria-describedby=\"$captionId\"", "aria-describedby=\"$captionId-{$uniqueId}\"", $output);
      }
      return $output;
		}
}
