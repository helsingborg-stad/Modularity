...
class App
{
    ...
    public function enqueueAdmin()
    {
        ...
        if (
            $modulesEditorId = \Modularity\Helper\Wp::isGutenbergEditor()
        ) {
            wp_register_script(
    ...
    public function isModularityPage()
    {
        ...
        if (
            strpos($current_screen->id, 'modularity') === false
        ...
        $options = get_option('modularity-options');
        if (isset($options['enabled-post-types']) && !in_array(get_post_type(), $options['enabled-post-types'])) {
            $result = false;
        }
        return $result;
    }
