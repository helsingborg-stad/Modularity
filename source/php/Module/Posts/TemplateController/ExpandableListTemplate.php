...
class ExpandableListTemplate
{
    ...
    public function __construct(\Modularity\Module\Posts\Posts $module, array $args, $data)
    {
        ...
        $this->data['posts_list_column_titles'] = $fields->posts_list_column_titles;
        ...
        $this->data['allow_freetext_filtering'] = $fields->allow_freetext_filtering;
            ...
            if ($this->data['posts_data_source'] === 'input') {
                if ($post->column_values !== false && count($post->column_values) > 0) {
                    foreach ($post->column_values as $key => $columnValue) {
                        $column_values[sanitize_title($posts_list_column_titles[$key]->column_header)] = $columnValue->value;
                    }
                }
            } else {
